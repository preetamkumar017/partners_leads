...
class _LeadsListState extends State<LeadsList> {
  ...
  void dispose() {
    super.dispose();
    leadsViewModel.dispose();
    deleteViewModel.dispose();
    dateInput.dispose();
    search.dispose();
                  ...
                  children: [
                    Expanded(
                      child: TextFormField(
                        ...
                        // onSaved: (value) => leadsViewModel.searchdata(value),
                      ),
                    ),
                    IconButton(
                        onPressed: () {
                          leadsViewModel.searchdata(search.text);
                        },
              ...
              Expanded(
                child: ChangeNotifierProvider<LeadsViewModel>.value(
                    ...
                    child: Consumer<LeadsViewModel>(builder: (context, value, _) {
                      switch (value.search.status) {
                        case Status.LOADING:
                          return const Center(
                              child: CircularProgressIndicator());
                        case Status.ERROR:
                          return Center(
                              child: Text(value.search.message.toString()));
                        case Status.COMPLETED:
                          leadsViewModel.setTotalLeads(value.search.data!.result!.length);
                          return RefreshIndicator(
                            onRefresh: () => leadsViewModel.fetchLeadsApi(data, context),
                            child: ListView.builder(
                              physics: bouncingScrollPhysics,
                              itemCount: value.search.data!.result!.length,
                              itemBuilder: (context, index) {
                                var result = value.search.data!.result![index];
                                // leadsViewModel.setTotalLeads(value.search.data!.result!.length);
              ...
              child: ChangeNotifierProvider<LeadsViewModel>.value(
               ...
               child: Consumer<LeadsViewModel>(
                builder: (context, provider, child) {
                      return Text("Total: ${leadsViewModel.getTotalLeads}",
                style: TextStyle(
                  color: Colors.white,
              ...
              child: Consumer<LeadsViewModel>(builder: (context, value, _) {
                if (MyStaticValue.emp_type == "1" ||
                  ...
                  return DropdownButtonFormField(
                    ...
                    items: value.search.data?.partnerNameId!.map(
                      (e) {
